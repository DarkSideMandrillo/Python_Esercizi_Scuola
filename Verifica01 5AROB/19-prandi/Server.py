        ...
        try:
            with sqlite3.connect("./fiumi.db", check_same_thread=False) as conn_db:
                ...
                message = self.connection.recv(
                    BUFFER_SIZE
                ).decode()  #!! riceve solo 1 messaggio
                ...
                while True:
                    ...
                    tempo_att = float(
                        tempo_att[1].replace(":", "")
                    )  # converto in float per far eil calcolo. #!! ma lo converti gia in client
                    if tempo_att - float(data) > 15:
                        ...
                        if (
                            livello_att > perc30 and livello_att < perc70
                        ):  #!! esistono gli elif
                            self.connection.sendall("avvenuta ricezione".encode())
                        ...
                        if livello_att > perc70:
                            ...
                            print(
                                f"Pericolo in corso nella stazione {dati[1]} del fiume {dati[0]}"
                            )
